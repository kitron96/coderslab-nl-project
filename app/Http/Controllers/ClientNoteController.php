...
class ClientNoteController extends AccountBaseController
{
    ...
    public function store(StoreClientNote $request)
    {
        ...
        return Reply::successWithData(__('messages.notesAdded'), ['redirectUrl' => in_array('client', user_roles()) ? route('client-notes.index') : route('clients.show', $note->client_id) . '?tab=notes']);
    }
    ...
    public function update(StoreClientNote $request, $id)
    {
        ...
        return Reply::successWithData(__('messages.notesUpdated'), ['redirectUrl' => in_array('client', user_roles()) ? route('client-notes.index') : route('clients.show', $note->client_id) . '?tab=notes']);
